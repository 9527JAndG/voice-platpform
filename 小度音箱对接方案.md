# 小度智能音箱对接方案 - 扫地机器人控制

## 📖 方案概述

小度智能音箱（DuerOS）是百度推出的智能语音助手平台，与天猫精灵类似，也采用 OAuth 2.0 授权 + Webhook 的方式进行智能家居设备对接。

## 🔄 对比分析：小度 vs 天猫精灵

| 对比项 | 小度音箱（DuerOS） | 天猫精灵（AliGenie） |
|--------|-------------------|---------------------|
| 开发平台 | 百度 DuerOS 开放平台 | 天猫精灵开放平台 |
| 授权协议 | OAuth 2.0 | OAuth 2.0 |
| 通信方式 | HTTPS + JSON | HTTPS + JSON |
| 设备发现 | Discovery 接口 | Discovery 接口 |
| 设备控制 | Control 接口 | Control 接口 |
| 核心差异 | 请求/响应格式略有不同 | 请求/响应格式略有不同 |

## 🎯 核心流程（与天猫精灵相似）

### 1. OAuth 授权流程

```
用户 → 小度 App → 授权请求
                    ↓
              /authorize (展示授权页面)
                    ↓
              用户确认授权
                    ↓
              生成 code 并重定向
                    ↓
              /token (code换token)
                    ↓
              返回 access_token
```

### 2. 设备发现流程

```
用户: "小度小度，发现设备"
         │
         ▼
    DuerOS 平台
         │
         ▼
    POST /smarthome/discovery
    Header: Authorization: Bearer {token}
         │
         ▼
    验证 Token → 查询用户设备 → 返回设备列表
```

### 3. 设备控制流程

```
用户: "小度小度，打开扫地机器人"
         │
         ▼
    DuerOS 平台 (解析语音指令)
         │
         ▼
    POST /smarthome/control
    {
      "header": {...},
      "payload": {"deviceId": "robot_001", "action": "turnOn"}
    }
         │
         ▼
    验证 Token → 执行设备操作 → 返回结果
```

## 📋 开发步骤

### 步骤 1: 准备工作

#### 1.1 注册开发者账号
- 访问百度 DuerOS 开放平台
- 注册并认证开发者账号
- 创建智能家居技能

#### 1.2 环境准备
```bash
- JDK 1.8+
- MySQL 8.0+
- Maven 3.6+
- 公网 HTTPS 地址（生产环境必须）
```

### 步骤 2: 复用天猫精灵项目架构

由于小度音箱和天猫精灵的对接方式非常相似，我们可以复用现有的项目架构：

#### 2.1 保持不变的部分
- ✅ OAuth 2.0 授权流程（完全相同）
- ✅ 数据库设计（完全相同）
- ✅ Service 层业务逻辑（基本相同）
- ✅ Repository 层（完全相同）
- ✅ 授权页面（完全相同）

#### 2.2 需要调整的部分
- 🔄 请求/响应 DTO 格式（略有差异）
- 🔄 Controller 层接口适配
- 🔄 设备类型和属性映射

### 步骤 3: 创建小度专用 Controller

创建 `DuerOSController.java`，适配小度的请求格式：

```java
@RestController
@RequestMapping("/dueros")
public class DuerOSController {
    
    @Autowired
    private OAuthService oauthService;
    
    @Autowired
    private DeviceService deviceService;
    
    /**
     * 设备发现接口（小度格式）
     */
    @PostMapping("/discovery")
    public ResponseEntity<?> discovery(
            @RequestHeader("Authorization") String authorization,
            @RequestBody DuerOSRequest request) {
        
        // 验证 Token
        String accessToken = extractToken(authorization);
        if (!oauthService.validateAccessToken(accessToken)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(createErrorResponse("INVALID_TOKEN"));
        }
        
        // 调用设备服务
        Long userId = 1L; // 从 token 获取用户ID
        DuerOSResponse response = deviceService.discoveryForDuerOS(userId);
        
        return ResponseEntity.ok(response);
    }
    
    /**
     * 设备控制接口（小度格式）
     */
    @PostMapping("/control")
    public ResponseEntity<?> control(
            @RequestHeader("Authorization") String authorization,
            @RequestBody DuerOSRequest request) {
        
        // 验证 Token
        String accessToken = extractToken(authorization);
        if (!oauthService.validateAccessToken(accessToken)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(createErrorResponse("INVALID_TOKEN"));
        }
        
        // 解析控制指令
        String deviceId = request.getPayload().get("deviceId").toString();
        String action = request.getPayload().get("action").toString();
        
        // 执行控制
        DuerOSResponse response = deviceService.controlForDuerOS(deviceId, action);
        
        return ResponseEntity.ok(response);
    }
}
```

### 步骤 4: 创建小度专用 DTO

#### 4.1 DuerOS 请求对象

```java
@Data
public class DuerOSRequest {
    private Header header;
    private Map<String, Object> payload;
    
    @Data
    public static class Header {
        private String namespace;
        private String name;
        private String messageId;
        private String payloadVersion;
    }
}
```

#### 4.2 DuerOS 响应对象

```java
@Data
public class DuerOSResponse {
    private Header header;
    private Map<String, Object> payload;
    
    @Data
    public static class Header {
        private String namespace;
        private String name;
        private String messageId;
        private String payloadVersion = "1.0";
    }
    
    public static DuerOSResponse success(String namespace, String name, String messageId) {
        Header header = new Header();
        header.setNamespace(namespace);
        header.setName(name);
        header.setMessageId(messageId);
        
        Map<String, Object> payload = new HashMap<>();
        
        DuerOSResponse response = new DuerOSResponse();
        response.setHeader(header);
        response.setPayload(payload);
        
        return response;
    }
}
```

### 步骤 5: 扩展 DeviceService

在现有的 `DeviceService` 中添加小度专用方法：

```java
/**
 * 设备发现 - 小度格式
 */
public DuerOSResponse discoveryForDuerOS(Long userId) {
    List<Device> devices = deviceRepository.findByUserId(userId);
    
    DuerOSResponse response = DuerOSResponse.success(
        "DuerOS.ConnectedHome.Discovery",
        "DiscoverAppliancesResponse",
        UUID.randomUUID().toString()
    );
    
    List<Map<String, Object>> discoveredAppliances = new ArrayList<>();
    
    for (Device device : devices) {
        Map<String, Object> appliance = new HashMap<>();
        appliance.put("applianceId", device.getDeviceId());
        appliance.put("manufacturerName", "自定义品牌");
        appliance.put("modelName", "扫地机器人");
        appliance.put("version", "1.0");
        appliance.put("friendlyName", device.getDeviceName());
        appliance.put("friendlyDescription", "智能扫地机器人");
        appliance.put("isReachable", true);
        
        // 支持的操作
        List<String> actions = Arrays.asList(
            "turnOn", "turnOff", "pause", "continue", "setMode"
        );
        appliance.put("actions", actions);
        
        // 设备类型
        appliance.put("applianceTypes", Arrays.asList("ROBOT_CLEANER"));
        
        discoveredAppliances.add(appliance);
    }
    
    response.getPayload().put("discoveredAppliances", discoveredAppliances);
    
    return response;
}

/**
 * 设备控制 - 小度格式
 */
public DuerOSResponse controlForDuerOS(String deviceId, String action) {
    Optional<Device> deviceOpt = deviceRepository.findByDeviceId(deviceId);
    
    if (!deviceOpt.isPresent()) {
        return createDuerOSError("DEVICE_NOT_FOUND", "设备不存在");
    }
    
    Device device = deviceOpt.get();
    
    // 根据 action 执行相应操作
    switch (action) {
        case "turnOn":
            device.setPowerState("on");
            break;
        case "turnOff":
            device.setPowerState("off");
            break;
        case "pause":
            // 暂停逻辑
            break;
        case "continue":
            // 继续逻辑
            break;
        default:
            return createDuerOSError("UNSUPPORTED_ACTION", "不支持的操作");
    }
    
    deviceRepository.save(device);
    
    return DuerOSResponse.success(
        "DuerOS.ConnectedHome.Control",
        action + "Confirmation",
        UUID.randomUUID().toString()
    );
}
```

### 步骤 6: 配置小度开放平台

#### 6.1 创建技能
1. 登录百度 DuerOS 开放平台
2. 创建"智能家居"技能
3. 填写技能基本信息

#### 6.2 配置 OAuth
```
授权地址: https://your-domain.com/authorize
Token 地址: https://your-domain.com/token
Client ID: xiaodu_client_id
Client Secret: xiaodu_client_secret
```

#### 6.3 配置 Webhook
```
设备发现接口: https://your-domain.com/dueros/discovery
设备控制接口: https://your-domain.com/dueros/control
```

### 步骤 7: 数据库配置

在现有数据库中添加小度客户端：

```sql
INSERT INTO oauth_clients (client_id, client_secret, redirect_uri, user_id) 
VALUES ('xiaodu_client_id', 'xiaodu_client_secret', 'https://dueros.baidu.com/callback', 1);
```

### 步骤 8: 测试流程

#### 8.1 OAuth 授权测试
```bash
# 访问授权页面
curl "http://localhost:8080/authorize?client_id=xiaodu_client_id&redirect_uri=https://dueros.baidu.com/callback&state=xyz&response_type=code"
```

#### 8.2 获取 Token
```bash
curl -X POST "http://localhost:8080/token" \
  -d "grant_type=authorization_code" \
  -d "client_id=xiaodu_client_id" \
  -d "client_secret=xiaodu_client_secret" \
  -d "code=YOUR_CODE"
```

#### 8.3 设备发现测试
```bash
curl -X POST "http://localhost:8080/dueros/discovery" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "header": {
      "namespace": "DuerOS.ConnectedHome.Discovery",
      "name": "DiscoverAppliancesRequest",
      "messageId": "test-001",
      "payloadVersion": "1.0"
    },
    "payload": {}
  }'
```

#### 8.4 设备控制测试
```bash
curl -X POST "http://localhost:8080/dueros/control" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "header": {
      "namespace": "DuerOS.ConnectedHome.Control",
      "name": "TurnOnRequest",
      "messageId": "test-002",
      "payloadVersion": "1.0"
    },
    "payload": {
      "deviceId": "robot_001",
      "action": "turnOn"
    }
  }'
```

## 🎮 语音控制示例

配置完成后，可以使用以下语音命令：

- "小度小度，打开扫地机器人"
- "小度小度，关闭扫地机器人"
- "小度小度，暂停扫地机器人"
- "小度小度，继续扫地"
- "小度小度，扫地机器人切换到自动模式"

## 📊 请求响应格式对比

### 天猫精灵格式
```json
{
  "header": {
    "namespace": "AliGenie.Iot.Device.Control",
    "name": "TurnOn",
    "messageId": "xxx"
  },
  "payload": {
    "deviceId": "robot_001"
  }
}
```

### 小度音箱格式
```json
{
  "header": {
    "namespace": "DuerOS.ConnectedHome.Control",
    "name": "TurnOnRequest",
    "messageId": "xxx",
    "payloadVersion": "1.0"
  },
  "payload": {
    "deviceId": "robot_001",
    "action": "turnOn"
  }
}
```

## 🔧 关键差异点

### 1. Namespace 命名
- 天猫精灵: `AliGenie.Iot.Device.*`
- 小度音箱: `DuerOS.ConnectedHome.*`

### 2. 请求名称
- 天猫精灵: `TurnOn`, `TurnOff`
- 小度音箱: `TurnOnRequest`, `TurnOffRequest`

### 3. Payload 结构
- 天猫精灵: 直接包含 `deviceId`
- 小度音箱: 包含 `deviceId` 和 `action`

### 4. 设备类型定义
- 天猫精灵: `robot_cleaner`
- 小度音箱: `ROBOT_CLEANER`

## 🚀 快速迁移方案

### 方案 A: 统一适配层（推荐）

创建一个适配器层，统一处理两个平台的差异：

```java
@Service
public class VoiceAssistantAdapter {
    
    public UnifiedResponse handleRequest(String platform, UnifiedRequest request) {
        if ("aligenie".equals(platform)) {
            return handleAliGenieRequest(request);
        } else if ("dueros".equals(platform)) {
            return handleDuerOSRequest(request);
        }
        throw new UnsupportedOperationException("不支持的平台");
    }
}
```

### 方案 B: 独立 Controller（简单直接）

为每个平台创建独立的 Controller：
- `AligenieController` - 天猫精灵
- `DuerOSController` - 小度音箱

## 📝 开发检查清单

- [ ] 创建 DuerOSController
- [ ] 创建 DuerOS DTO 类
- [ ] 扩展 DeviceService 添加小度方法
- [ ] 在数据库中添加小度客户端
- [ ] 配置小度开放平台
- [ ] 测试 OAuth 授权流程
- [ ] 测试设备发现
- [ ] 测试设备控制
- [ ] 在小度 App 中测试语音控制

## 🎯 总结

小度音箱的对接方案与天猫精灵非常相似，主要差异在于：

1. **请求/响应格式**: Namespace 和字段名称略有不同
2. **设备类型定义**: 命名规范不同
3. **平台配置**: 需要在不同的开放平台配置

通过复用现有的天猫精灵项目架构，只需要：
- 添加小度专用的 Controller 和 DTO
- 扩展 DeviceService 添加格式转换方法
- 在数据库中添加小度客户端配置

即可快速完成小度音箱的对接，实现一套后端同时支持多个语音平台！
